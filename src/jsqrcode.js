...
(function (root) {
    ...
    qrcode.process = function (n) {
        ...
        var cw = [0,1,2].map(function(x){return qrcode.grayScaleToBitmap(qrcode.getLayer(x))});
        var wwq = [0,1,2].map(function(x){
          var c = cw[x];
    ...
    qrcode.getPixelC = function (a, b, c) {
        if (qrcode.width < a) {
            throw("point error");
        }
        if (qrcode.height < b) {
        ...
        var point = (a * 4) + (b * qrcode.width * 4);
        var p = qrcode.imagedata.data[point+c];
        return p
    };
    qrcode.getLayer = function(cw){
          var c = new Array(qrcode.width * qrcode.height);
          for (var d = 0; d < qrcode.height; d++) {
              for (var b = 0; b < qrcode.width; b++) {
                  var a = qrcode.getPixelC(b, d, cw);
                  c[b + d * qrcode.width] = a
              }
          }
          if(qrcode.debug) Layer2ImageData(c,cw);
          return c
    };
    var Layer2ImageData = function(layer,cw){
        var c = document.querySelector("#l"+cw);
        var cx = c.getContext('2d');
        var d = cx.getImageData(0,0,c.width,c.height);
        for (var a = 0; a < qrcode.height; a++) {
            for (var b = 0; b < qrcode.width; b++) {
              var point = (b * 4) + (a * qrcode.width * 4);
                  d.data[point + 0] = layer[b + a * qrcode.width];
                  d.data[point + 1] = layer[b + a * qrcode.width];
                  d.data[point + 2] = layer[b + a * qrcode.width];
                  d.data[point + 3] = 255;
            }
        }
